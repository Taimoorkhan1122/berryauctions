...
const AuctionInProgress = auctionData.slice(0,4).map((item, index) => (
  <AuctionCard key={index + "_01"} data={item} />
));
const MajorWork = majorWorksData
  .slice(0, 4)
  .map((item, index) => <MajorWorkCard key={index + "_01"} data={item} />);
const Artists = artistData
  .slice(0, 4)
  .map((item, index) => <ArtistCard key={index + "_01"} {...item} />);
const Auctions = () => {
